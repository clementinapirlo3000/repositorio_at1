<?php 
/**
  * Nombre: consulta_establecimientos.php
  * Autor: Ing: Fernando Paez Acero
  * Autor: Ing: Alex Castro
  * Autor: Ing: Carlos Chacon
  * Acces public
  * Copyleft: 2017
  * Formulario para la consulta de establecimientos
  *
**/
...
if ($eliminar_get > 0)
{
  ...
  $insertar_usuario = "UPDATE rrh_testb SET fk_id_rrh_estat = 2 WHERE id_rrh_testb = $id_del_usuario2";
  $usuarios_objeto2_conexion_BD->ejecutar_sql($insertar_usuario);
}
 $select_objeto_conexion_BD  = new clase_conecta_postgresql;
 $consulta_zopos_existente= "SELECT * FROM rrh_testb WHERE fk_id_rrh_estat != 2";
...
<div class="col-xs-1">
  <button type="button" onClick="document.location.href='../vista/establecimientos.php';" class="btn btn-primary btn-xs">Agregar</button>
</div>
<div class="col-xs-1">
...
<form>
  <table id="example" class="display" cellspacing="0" width="100%">
    ...
    <tbody> <?PHP
      ...
      { echo "
        <tr>
          <td>".$registro_zpos['tes_nombre']."</td>
          <td align='center'>
            <a href='../vista/mod_establecimientos.php?id=$registro_zpos[id_rrh_testb]'>
              <button type='button' class='btn btn-info btn-xs'>
                <span class='input-group' id='basic-addon1'>
                 <i class='fa fa-pencil fa-lg' aria-hidden='true'></i>
                </span>
              </button>
            <a href='consulta_establecimientos.php?id=$registro_zpos[id_rrh_testb]'>
...
////// Se registra en la bitacora la acción del usuario             //
  ...
  $descripcion_ejecucion = "consulta_establecimientos.php";
  $movimiento_bitacora = $_SESSION['idusuarios$nom_proyec'].' '.$_SESSION['nombre1$nom_proyec'].' '.$_SESSION['apellido1$nom_proyec'].'...'."CONSULTANDO DATOS DE LOS ESTABLECIMIENTOS";
  $objeto_bitacora = "CONSULTANDO DATOS DE LOS ESTABLECIMIENTOS";
