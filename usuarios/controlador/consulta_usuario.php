<?php 
/**
  * Nombre: consulta_usuario.php
  * Autores: Ing: Fernando Paez Acero, Ing: Alex Castro, Ing: Carlos Chacon
  * Acces public
  * Copyleft: 2017
  * Formulario para la consulta de usuarios del sistema
  *
**/
...
if ($eliminar_get > 0)
{
  ...
  $usuarios_objeto2_conexion_BD  = new clase_conecta_mysql;
...
$select_objeto_conexion_BD  = new clase_conecta_postgresql;
 $consulta_zopos_existente= "SELECT * FROM rrh_usuar usu, rrh_codce cod WHERE usu.fk_id_rrh_codce = cod.id_rrh_codce AND usu.fk_id_rrh_estat != 2";
...
<h2>USUARIOS</h2>
<div class="col-xs-1">
  <button type="button" onClick="document.location.href='../vista/usuarios.php';" class="btn btn-primary btn-xs">Agregar</button>
</div>
<div class="col-xs-1">
...
<form>
  <table id="example" class="display" cellspacing="0" width="100%">
    ...
    <tbody> <?PHP
      ...
      { echo "
        <tr>
          <td>".$registro_zpos['usu_cedul']."</td>
          <td>".$registro_zpos['usu_nomb1'].' '.$registro_zpos['usu_nomb2']."</td>
          <td>".$registro_zpos['usu_apel1'].' '.$registro_zpos['usu_apel2']."</td>
          <td>".$registro_zpos['cod_celul'].'-'.$registro_zpos['usu_tecel']."</td>
          <td>".$registro_zpos['usu_email']."</td>
          <td align='center'>
            <a href='../vista/mod_usuarios.php?id=$registro_zpos[id_rrh_usuar]'>
              <button type='button' class='btn btn-info btn-xs'>
                <span class='input-group' id='basic-addon1'>
                 <i class='fa fa-pencil fa-lg' aria-hidden='true'></i>
                </span>
              </button>
            <a href='../vista/consulta_usuarios.php?id=$registro_zpos[id_rrh_usuar]'>
...
////// Se registra en la bitacora la acción del usuario             //
  ...
  $descripcion_ejecucion = "consulta_usuarios.php";
  $movimiento_bitacora = $_SESSION['idusuarios$nom_proyec'].' '.$_SESSION['nombre1$nom_proyec'].' '.$_SESSION['apellido1$nom_proyec'].'...'."CONSULTANDO DATOS DEL USUARIO";
  $objeto_bitacora = "CONSULTANDO DATOS DEL USUARIO DEL SISTEMA";
